...
<title>공연사 관리창(목록, 등록, 수정, 삭제)</title>
<script>
	function deletePartner(frm) {
	    var isDelete = confirm("삭제 하시겠습니까?");
	    if (isDelete) {
	       frm.action= "deletePartner.do";
	       frm.submit();
	    } else {
	       return;
	    }
...
<div>
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<td>${part.phone }</td>
			<td>${part.email }</td>
			<td class="center">${part.address}</td>
			<td><input class="btn btn-secondary" type="button" value="수정" onclick="javascript:location.href='updatePartner.jsp'"></td>
			<td><input class="btn btn-secondary" type="button" value="삭제" onclick="deletePartner(this.form)"></td>
		</tr>
		</c:forEach>
	</table>
		<input class="btn btn-secondary" type="button" value="공연사등록" onclick="javascript:location.href='insertPartner.jsp'">
	</div>
