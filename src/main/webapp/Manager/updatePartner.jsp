...
<title>공연사 수정</title>
<script>
	function updatePartner(frm) {
	    var isDelete = confirm("수정 하시겠습니까?");
	    if (isDelete) {
	       frm.action= "updatePartner.do";
	       frm.submit();
	    } else {
	       return;
	    }
...
<div>
	<!-- 검색 기능 -->
	<form action="PartnerList.do" method="post">
	<div class="container">
	...
	<h4>공연사 수정 페이지</h4>
		<tr>
			<td>
				...
				<c:forEach var="option" items="${P_conditionMap}">
	...
	<table class="table table-bordered">
		<tr>
			...
			<th width="100" style="text-align: left;">주소</th>
			<th></th>
		</tr>
		<c:forEach var="part" items="${partnerList }">
		<form method="post">
		<tr>
			<td class="center" >${part.p_id }</td>
			<td><input type="text" name="partner_name" value="${part.partner_name }"></td>
			<td><input type="text" name="phone" value="${part.phone }"></td>
			<td><input type="text" name="email" value="${part.email }"></td>
			<td><input type="text" name="address" value="${part.address}"></td>
			<td><input class="btn btn-secondary" type="button" value="수정" onclick="updatePartner(this.form)"></td>
		</form>
		</c:forEach>
	</table>
			<input class="btn btn-secondary" type="button" value="목록" onclick="javascript:location.href='getPartnerList.jsp'">
			<input type="hidden" name="p_id" value="${part.p_id}">
	</div>
	</form>
	<div style="height: 20px;"></div>
</div>
